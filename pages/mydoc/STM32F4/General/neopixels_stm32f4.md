...
permalink: neopixels_stm32f4.html
folder: mydoc
comments: false
toc: false
datatable: true
...
* t0 is the high time for a 0 bit in nanoseconds
* t1 is the high time for a 1 bit in nanoseconds
* tc is the total cycle length (on plus off time) in nanoseconds
...
So, to set up a neopixel that needs a 0 time of 300ns, a 1 time of 800ns, a cycle time of 1250ns, and a reset time of 250,000ns (or 250us), you would use:  
...
Do not try to update the neopixels during motion, as this will cause a momentary pause in movement.  
...
The process to control neopixels from 3.4.1-RC1 has changed from using bit-banging to using Hardware DMA. This uses hardware to change the colour of the neopixels, allowing for colours to be changed whilst movement is taking place. 
