...
permalink: firmware_build_stm32f4.html
folder: mydoc
comments: false
toc: false
---
...
4. Then download and install [msys2](https://www.msys2.org/). Once installed you will need to use MSYS2 MSYS to install make by using the command `pacman -S make`. Then add the location of the bin folder that is installed in to your windows path environment setting (typically found in `C:\msys64\usr\bin\make.exe`). Make sure you add them to the system variables and not the user variables. While there also add the location of mkdir.exe, which will have been installed with git. It can be found in the usr/bin folder where git is installed.  
...
Download the following github repositories. The preferred method for doing so is by using a git client (terminal or GUI). This allows for changes made to be tracked. My client of choice (jay_s_uk) is [GitKraken](https://www.gitkraken.com/).  
