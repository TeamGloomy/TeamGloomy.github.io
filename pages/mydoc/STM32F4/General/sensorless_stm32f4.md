...
permalink: sensorless_stm32f4.html
folder: mydoc
toc: false
comments: false
---

Sensorless homing and stall detection is functional for the TMC2209s and TMC2226s. This is supported on the following boards.
...
The correct pins, in the correct order, have already been included in the board.txt section of each supported board's wiki page, for both WiFi and SBC configurations.  
...
Before using the examples below to configure sensorless homing, it is suggested to test each axis with the following homing code. Ensure that you comment out your current homing code using `;`.  
...
Once you have confirmed that your motors are stalling, adapt the example Cartesian homing files below. Examples of CoreXY sensorless homing files can also be found below.  
...
{% include tip.html content="RRF 3.4.x and above includes M201.1 (set reduced acceleration for special move types). Lowering the acceleration of your homing moves with M201.1 might increase sensorless-homing reliability. On prior versions, you can decrease and restore your acceleration values manually inside homex.g/homey.g." %}
...
If your drivers stall straight away with the example homing files, increase the S value.  
