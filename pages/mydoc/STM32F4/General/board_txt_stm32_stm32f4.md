...
permalink: board_txt_stm32_stm32f4.html
folder: mydoc
comments: false
toc: false
datatable: true
...
| :------------- | :---------------|:---|:---|:---|
|wifi.clockReg|wifi.clockReg = 8195|Sets the WiFi clock speed. ESP32 only. The default is 8195 (DMA 20MHz - Typical WiFi transfer speed of 1.56Mbytes/S for F4). F4 processors seem to work ok at 8194 (DMA 27MHz - Typical WiFi transfer speed of 1.80Mbytes/S for F4). 3.4.1 onwards and requires DuetWifiServer 1.26_09 onwards||
|wifi.csPin|wifi.csPin = PE_12|Allow use of non standard CS pin||
|wifi.espDataReadyPin|example wifi.espDataReadyPin = PE_13| used to indicate that data is available from the ESP8266||
|wifi.espResetPin|wifi.espResetPin = PE_15|This sets the reset pin to use when connecting an ESP8266||
|wifi.moduleType|wifi.moduleType = esp32| This sets the type of module installed so the correct firmware is installed. esp32, esp32eth and esp8266 are valid options||
|wifi.TfrReadyPin|wifi.TfrReadyPin = PE_14|This sets the transfer ready pin when connecting an ESP8266||
|wifi.serialRxTxPins|wifi.serialRxTxPins = { PD_9, PD_8 }|This sets the pin numbers to use for RX and TX when connecting an ESP8266. See table below for allowable RX and TX pins||
|wifi.spiChannel|wifi.spiChannel=1|This allows the SPI channel to be set that is used for WiFi. The default is 1. It can only be set to channel 1 (SPI2) or channel 2 (SPI3). From 3.3rc2_2 only||
...
|lcd.spiChannel|lcd.spiChannel = 255|This selects which SPI channel is used to communicate with the LCD. Can be set to 255 to indicate no channel to be used|STM32F4 has six SPI interfaces (three channels 0, 1, 2 are hardware based although only 0 and 1 are implemented, channels 3, 4 and 5 are software).|
...
|leds.activityOn|leds.activityOn = 1|This sets the polarity of the activity LED and is a boolean value|3.4.2-RC2 Onwards|
|leds.diagnostic|leds.diagnostic = PA_13|This would set the correct pin for controlling a diagnostic LED||
|leds.diagnosticOn|leds.diagnosticOn = 1|This sets the polarity of the diagnostic LED and is a boolean value|3.4.2-RC2 Onwards|
