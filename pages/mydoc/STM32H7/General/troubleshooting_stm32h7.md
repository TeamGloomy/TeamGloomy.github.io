...
Here you may find some instructions on how to troubleshoot the most common problems when setting up this port of RepRapFirmware for STM32.
...
- Make sure you are using an SD-card in FAT32 format. exFAT, NTFS, or any other format is not supported.
- Make sure you've renamed the firmware file to `firmware.bin` and not `firmware.bin.bin`
...
This can be checked by sending `M122 P200` in a USB terminal. Search for the lines `wifi.serialRxTxPins` and `serial.aux.rxTxPins` under the *Configurable Board.txt Settings* section.  
Please refer to the *Connecting via WiFi* section for your board in the sidebar for the correct values to use.

## Wi-Fi issues

- Make sure the board is powered through 12V/24V.
- Make sure you are using a proper USB cable and not a charging cable (which doesn't have any data wires).  
If you encounter issues connecting to your board through USB, first make sure the board is flashed and then try another USB cable.
...
If you are having trouble adding your SSID, try a simplified version with only letters and numbers.
