...
4. Then download and install [msys2](https://www.msys2.org/). Once installed you will need to use MSYS2 MSYS to install make by using the command `pacman -S make`. Then add the location of the bin folder that is installed in to your windows path environment setting (typically found in `C:\msys64\usr\bin\make.exe`). Make sure you add them to the system variables and not the user varaibles. While there also add the location of mkdir.exe, which will have been installed with git. It can be found in the usr/bin folder where git is installed.  
